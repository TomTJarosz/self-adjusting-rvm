...
eye_cascade = cv2.CascadeClassifier('../../TestEyeTracking/opencv/data/haarcascades/haarcascade_eye.xml')
...
def visualize_boxes_and_labels_on_image_array(image,
  ...
  for box, color in box_to_color_map.items():
    ...
    print 'size='+str((ymax-ymin)*(xmax-xmin))
    if instance_masks is not None:
    ...
    print 'pre_eyes='+str(pre_eyes)
    if len(pre_eyes)>2:
      print "yoooooo"
      exes=np.copy(pre_eyes[:,1])
      index1=np.argmin(exes)
      exes[index1]=shape1+shape0
      index2=np.argmin(exes)
      eyes=[]
    ...
    for (ex,ey,ew,eh) in eyes:
      ...
      draw_bounding_box_on_image_array(
        image,
        float(minye+ey)/float(shape0),
        float(minxe+ex)/float(shape1),
        float(minye+ey+eh)/float(shape0),
        float(minxe+ex+ew)/float(shape1),
        color=color,
        thickness=line_thickness,
        display_str_list=box_to_display_str_map[box],
        use_normalized_coordinates=use_normalized_coordinates)
