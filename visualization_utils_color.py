...
def visualize_boxes_and_labels_on_image_array(image,
  ...
  for imgnum in range(1):
    shape0=image[0].shape[0]
    shape1=image[0].shape[1]
    boxes=boxess[imgnum][0:16]
    scores=scoress[imgnum][0:16]
    ...
    for ind in xrange(4):
        ...
        gray = cv2.cvtColor(image[imgnum], cv2.COLOR_BGR2GRAY)
...
def bad_intersect(i1,i2):
    ...
    toplefty=np.max([i1[1],i2[1]])
    botrightx=np.min([i1[0]+i1[2],i2[0]+i2[2]])
    botrighty=np.min([i1[1]+i1[3],i2[1]+i2[3]])
    if botrightx<=topleftx or botrighty<=toplefty:
        return False
    inter_area=(botrightx-topleftx)*(botrighty-toplefty)
    i1_area=i1[2]*i1[3]
    i2_area=i2[2]*i2[3]
    if i1_area/2.0<inter_area or i2_area/2.0<inter_area:
        return True
    return False